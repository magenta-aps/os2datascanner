...
from .views.webscanner_views import (
    WebScannerCreate, WebScannerUpdate, WebScannerDelete, WebScannerRun,
    WebScannerAskRun, WebScannerList, WebScannerValidate)

urlpatterns = [
    # App URLs
    url(r'^$', MainPageView.as_view(), name='index'),
    url(r'^exchangescanners/$',
        ExchangeScannerList.as_view(),
        name='exchangescanners'),
    url(r'^exchangescanners/add/$',
    ...
    url(r'^webscanners/add/$',
        WebScannerCreate.as_view(),
        name='webscanner_add'),
    url(r'^webscanners/(?P<pk>\d+)/delete/$',
        WebScannerDelete.as_view(),
        name='scanner_delete'),
    url(r'^webscanners/(?P<pk>\d+)/validate/$',
        WebScannerValidate.as_view(),
        name='web_scanner_validate'),
    url(r'^webscanners/(?P<pk>\d+)/run/$',
        WebScannerRun.as_view(),
        name='webscanner_run'),
    url(r'^webscanners/(?P<pk>\d+)/askrun/$',
        WebScannerAskRun.as_view(
            ...
            model=WebScanner),
        name='scanner_askrun'),
    url(r'^webscanners/(?P<pk>\d+)/$',
        WebScannerUpdate.as_view(),
        name='scanner_update'),
    url(r'^filescanners/$', FileScannerList.as_view(), name='filescanners'),
    url(r'^filescanners/add/$',
        FileScannerCreate.as_view(),
        name='filescanner_add'),
    url(r'^filescanners/(?P<pk>\d+)/$',
        FileScannerUpdate.as_view(),
        name='scanner_update'),
    url(r'^filescanners/(?P<pk>\d+)/delete/$',
        FileScannerDelete.as_view(),
        name='scanner_delete'),
    url(r'^filescanners/(?P<pk>\d+)/run/$',
        FileScannerRun.as_view(),
    ...
    url(r'^rules/cpr/(?P<pk>\d+)/$',
        CPRRuleUpdate.as_view(),
        name='rule_update'),
    url(r'^rules/cpr/(?P<pk>\d+)/delete/$',
        CPRRuleDelete.as_view(),
        name='rule_delete'),
    url(r'^rules/regex/add/$', RegexRuleCreate.as_view(),
        name='regexrule_add'),
    url(r'^rules/regex/(?P<pk>\d+)/$',
        RegexRuleUpdate.as_view(),
        name='rule_update'),
    url(r'^rules/regex/(?P<pk>\d+)/delete/$',
        RegexRuleDelete.as_view(),
        name='rule_delete'),
    url(r'^reports/$', ReportList.as_view(), name='reports'),
    url(r'^reports/web/$',
        ReportList.as_view(
            active='web',
            queryset=Scan.objects.filter(scanner__webscanner__isnull=False)),
        name='reports'),
    url(r'^reports/file/$',
        ReportList.as_view(
            active='file',
            queryset=Scan.objects.filter(scanner__filescanner__isnull=False)),
        name='reports'),
    url(r'^reports/exchange/$',
        ReportList.as_view(
            active='exchange',
            queryset=Scan.objects.filter(
                scanner__exchangescanner__isnull=False)),
        name='reports'),
    url(r'^report/(?P<pk>[0-9]+)/$', ReportDetails.as_view(), name='report'),
    url(r'^report/(?P<pk>[0-9]+)/full/$',
        ReportDetails.as_view(full=True),
        name='full_report'),
    url(r'^report/(?P<pk>[0-9]+)/csv/$',
        CSVReportDetails.as_view(),
        name='csvreport'),
    url(r'^report/(?P<pk>[0-9]+)/log/$',
        ScanReportLog.as_view(),
        name='logreport'),
    url(r'^report/(?P<pk>[0-9]+)/delete/$',
        ReportDelete.as_view(),
        name='report_delete'),
    url(r'^reports/summaries/$', SummaryList.as_view(), name='summaries'),
    url(r'^reports/summaries/add/$',
        SummaryCreate.as_view(),
        name='summary_add'),
    url(r'^reports/summary/(?P<pk>\d+)/$',
        SummaryUpdate.as_view(),
        name='summary_update'),
    url(r'^reports/summary/(?P<pk>\d+)/report/$',
        SummaryReport.as_view(),
        name='summary_report'),
    url(r'^reports/summary/(?P<pk>\d+)/delete/$',
        SummaryDelete.as_view(),
        name='summary_delete'),
    # Login/logout stuff
    ...
    url(r'^(rules/regex|rules/cpr|groups|reports/summaries)/(\d+)/(saved)/$',
        DialogSuccess.as_view()),
    url(r'^jsi18n/$',
        JavaScriptCatalog.as_view(packages=('os2webscanner', 'recurrence'), )),
    # System functions
    url(r'^system/status/?$', SystemStatusView.as_view()),
    url(r'^system/orgs_and_domains/$',
